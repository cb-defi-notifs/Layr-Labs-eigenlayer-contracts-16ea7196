...
contract DataLayr is Ownable, IDataLayr {
    ...
    //TODO: figure out how to set this appropriately. this is simply a placeholder for now.
    uint256 public signatureQuorumSize = 10;
...
//TODO: multiple indices for different things? e.g. one for ETH, one for EIGEN?
    ...
    */
    function confirm() public {
        ...
        bytes32 ferkleRoot;
        assembly {
            //get the 64 bits immediately after the function signature
            dumpNumber := shr(192, calldataload(4))
            //get the 32 bytes immediately after the above
            ferkleRoot  := calldataload(12)
            //get the next 16 bits
            numberOfBins  := shr(240, calldataload(44))
        }
        ...
        while(binsProcessed < numberOfBins) {
            ...
            for (uint16 i; i < sigsInCurrentBin; i++) {
                ...
                require(claimsMadeInBin & mask == 0, "claim already made on this bit - repeat signature?");
        ...
        //check that enough signatures have been provvided
        require(totalValidSignatures >= signatureQuorumSize, "not enough valid signatures!");
