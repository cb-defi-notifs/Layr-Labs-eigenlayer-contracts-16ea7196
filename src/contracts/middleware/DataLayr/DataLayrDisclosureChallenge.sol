...
contract DataLayrDisclosureChallenge {
    ...
    function challengePaymentHalf(
        ...
        assembly {
            if iszero(
                call(not(0), 0x06, 0, coors, 0x80, sum, 0x40)
            ) {
                revert(0, 0)
            }
        }
    ...
    function respondToPaymentChallengeFinal(
        ...
        require(challenge.increment == 0, "Time to do dissection proof");
        require(
            block.timestamp <
        ...
        if (turn) {
            ...
            resolve(headerHash, contest_point[0] != coors[3] || contest_point[1] != coors[4]);
        } else{
    ...
    function proveDegreeLeaf(bool[] calldata leftRightFlags, bytes32[] calldata proof) public returns(uint48) {
        ...
        require(len == dlsm.log2NumPowersOfTau() - 1, "Proof is not the correct length");
        //amount to increment when proved to the right
        uint48 increment = dlsm.numPowersOfTau()/2;
        ...
        for(uint i = 0; i < len;) {
            if(leftRightFlags[i]) {
                ...
                // [..., prevleftroot, prevrightroot, leftroot, rightroot, ...]
                require(proof[2*i] == keccak256(abi.encodePacked(proof[2*i+2], proof[2*i+3])), "Incorroct proof");
            } else {
                //right branch
                //            2*i          2*i+1        2*i+2     2*i+3
                // [..., prevleftroot, prevrightroot, leftroot, rightroot, ...]
                require(proof[2*i+1] == keccak256(abi.encodePacked(proof[2*i+2], proof[2*i+3])), "Incorroct proof");
                degree += increment;
                increment /= 2;
            }
            unchecked {
    ...
    function resolve(bytes32 headerHash, bool challengeSuccessful) internal {
        dlsm.resolveDisclosureChallenge(headerHash, challenge.operator, challengeSuccessful);
