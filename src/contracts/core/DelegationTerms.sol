...
contract DelegationTerms is IDelegationTerms, RepositoryAccess, DSTest {
    ...
    function payForService(IERC20 token, uint256 amount) external payable {
       ...
       (uint96 totalEthStaked, uint96 totalEigenStaked) = repository.registry().totalStake();
       (uint96 operatorEthStaked, uint96 operatorEigenStaked) = repository.registry().operatorStakes(operator);
...
//NOTE: currently this causes the delegator to lose any pending rewards
    ...
    function weightOfEth(address delegator) public returns(uint256) {
        // TODO: make this work better. right now I believe this is broken if delegator falls below node threshold
