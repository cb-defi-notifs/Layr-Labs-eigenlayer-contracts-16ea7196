...
contract EigenPodPaymentEscrow is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, Pausable, IEigenPodPaymentEscrow {
    ...
    function claimPayments(uint256 maxClaimsToMake) external nonReentrant onlyWhenNotPaused(PAUSED_WITHDRAWALS) {
        ...
        uint256 paymentsCompletedBefore = _userPayments[msg.sender].paymentsCompleted;
        uint256 maxIndex = _userPayments[msg.sender].payments.length - 1;
        uint256 i = 0;
        while (i < maxClaimsToMake && (paymentsCompletedBefore + i) < maxIndex) {
            // copy payment from storage to memory
            Payment memory payment = _userPayments[msg.sender].payments[paymentsCompletedBefore + i];
        ...
        _userPayments[msg.sender].paymentsCompleted = paymentsCompletedBefore + i;
        // actually send the ETH
        if (amountToSend != 0) {
            AddressUpgradeable.sendValue(payable(msg.sender), amountToSend);
        }
    }
