...
contract EigenPod is IEigenPod, Initializable, ReentrancyGuardUpgradeable, EigenPodPausingConstants, Test {
    ...
    {
        require(validatorStatus[validatorIndex] == VALIDATOR_STATUS.INACTIVE, "EigenPod.verifyCorrectWithdrawalCredentials: Validator must be inactive to prove withdrawal credentials");
        ...
        uint64 validatorCurrentBalanceGwei = Endian.fromLittleEndianUint64(proof.balanceRoot);
    ...
    function verifyOvercommittedStake(
       ...
       // ensure that the blockNumber being proven against is not "too stale".
        ...
        // if the validatorCurrentBalanceGwei is zero *and* the validator is overcommitted, then overcommitment should be proved through `verifyBeaconChainFullWithdrawal`
     ...
     * @param validatorProofs is the information needed to check the veracity of the validator being proven
    ...
    function verifyAndProcessWithdrawal(
        ...
        BeaconChainProofs.ValidatorFieldsAndBalanceProof calldata validatorProofs, 
        bytes32[] calldata validatorFields,
    ...
    {
        require(validatorStatus[withdrawalProofs.validatorIndex] == VALIDATOR_STATUS.ACTIVE, "EigenPod.verifyOvercommittedStake: Validator not active");
        ...
        BeaconChainProofs.verifyValidatorFields(uint40(withdrawalProofs.validatorIndex), beaconStateRoot, validatorProofs, validatorFields);
