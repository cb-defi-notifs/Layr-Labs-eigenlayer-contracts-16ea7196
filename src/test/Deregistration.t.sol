...
contract DeregistrationTests is TestHelper {
     ...
     */
    function testBLSDeregistration(uint8 operatorIndex, uint256 ethAmount, uint256 eigenAmount)
        public
        fuzzedOperatorIndex(operatorIndex)
    {
        ...
        _testDeregisterOperatorWithDataLayr(
            operatorIndex, pubkeyToRemoveAff, uint8(dlReg.numOperators() - 1), testEphemeralKey
        );

        (, uint32 nextUpdateBlockNumber, uint96 firstQuorumStake, uint96 secondQuorumStake) =
            dlReg.pubkeyHashToStakeHistory(pubkeyHash, dlReg.getStakeHistoryLength(pubkeyHash) - 1);
        ...
        require(
            currAPKHash == prevAPKHash, "aggregate public key has not been updated correctly following deregistration"
        );
    }
    ...
    function testDeregisteringAlreadyDeregisteredOperator(uint8 operatorIndex, uint256 ethAmount, uint256 eigenAmount)
        public
        fuzzedOperatorIndex(operatorIndex)
    {
        BLSRegistration(operatorIndex, ethAmount, eigenAmount);
    ...
    function BLSRegistration(uint8 operatorIndex, uint256 ethAmount, uint256 eigenAmount)
        internal
        fuzzedOperatorIndex(operatorIndex)
    {
        cheats.assume(ethAmount > 0 && ethAmount < 1e18);
