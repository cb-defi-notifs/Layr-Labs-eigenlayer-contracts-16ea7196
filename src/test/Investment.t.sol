...
contract InvestmentTests is EigenLayerTestHelper {
    ...
    function testPreventSlashing() public {
        ...
        IInvestmentStrategy strategy = wethStrat;
        ...
        cheats.expectRevert("InvestmentStrategyBase.withdraw: Can only withdraw the strategy token");
        investmentManager.slashQueuedWithdrawal(address(slasher), queuedWithdrawal, tokensArray, emptyUintArray);
        cheats.stopPrank();
        ...
        cheats.startPrank(staker);
        investmentManager.completeQueuedWithdrawal(queuedWithdrawal, tokensArray, 0, false);
        cheats.stopPrank();
    ...
    function testWithdrawalSequences() public {
        ...
        cheats.expectRevert("InvestmentManager.completeQueuedWithdrawal: shares pending withdrawal are still slashable");
        investmentManager.completeQueuedWithdrawal(queuedWithdrawal, tokensArray, correctMiddlewareTimesIndex, false);
        }
        ...
        cheats.expectRevert("InvestmentManager.completeQueuedWithdrawal: shares pending withdrawal are still slashable");
        investmentManager.completeQueuedWithdrawal(queuedWithdrawal, tokensArray, staleMiddlewareTimesIndex, false);
        }
        ...
        internal returns(bytes32 withdrawalRoot, IInvestmentManager.QueuedWithdrawal memory queuedWithdrawal)
    {
        ...
        //queue the withdrawal
        cheats.startPrank(staker);
        withdrawalRoot = investmentManager.queueWithdrawal(strategyIndexes, strategyArray, shareAmounts, withdrawer, true);
        cheats.stopPrank();
    ...
    function testFrontrunFirstDepositor(/*uint256 depositAmount*/) public {
        ...
        InvestmentManager _investmentManager = _whitelistStrategy(investmentManager, wethStrat);

        cheats.startPrank(attacker);
        weth.approve(address(investmentManager), type(uint256).max);
        cheats.expectRevert("InvestmentStrategyBase.deposit: updated totalShares amount would be nonzero but below MIN_NONZERO_TOTAL_SHARES");
        _investmentManager.depositIntoStrategy(wethStrat, weth, 1 wei);
        cheats.stopPrank();
        ...
        weth.approve(address(_investmentManager), type(uint256).max);
        _investmentManager.depositIntoStrategy(wethStrat, weth, 2 ether);
        cheats.stopPrank();

        //attacker deposited 1 ether and 1 wei - received 1 share 
        //user deposited 2 ether - received 1 share
        //user has lost 0.5 ether
        (, uint256[] memory shares) = _investmentManager.getDeposits(attacker);
        require(shares.length == 0, "Attacker deposit should fail due to minimum balances");

        (, shares) = _investmentManager.getDeposits(user);
        require(wethStrat.sharesToUnderlyingView(shares[0]) >= (1900000000000000000), "user has lost more than 0.1 eth from frontrunning");

    }

    function _whitelistStrategy(InvestmentManager _investmentManager, InvestmentStrategyBase _strategyBase) internal returns(InvestmentManager) {
        // whitelist the strategy for deposit
        cheats.startPrank(investmentManager.owner());
        IInvestmentStrategy[] memory _strategy = new IInvestmentStrategy[](1);
        _strategy[0] = IInvestmentStrategy(_strategyBase);
        _investmentManager.addStrategiesToDepositWhitelist(_strategy);
        cheats.stopPrank();
