...
contract RegistrationTests is TestHelper {
    ...
    function testBLSRegistration(uint8 operatorIndex, uint256 ethAmount, uint256 eigenAmount)
        public
        fuzzedOperatorIndex(operatorIndex)
    {
        cheats.assume(ethAmount > 0 && ethAmount < 1e18);
        cheats.assume(eigenAmount > 0 && eigenAmount < 1e18);

        uint8 operatorType = 3;
        (uint256 amountEthStaked, uint256 amountEigenStaked) =
            _testInitiateDelegation(operatorIndex, eigenAmount, ethAmount);
        ...
        {
            uint96 ethStakedBefore =
                dlReg.getTotalStakeFromIndex(dlReg.getLengthOfTotalStakeHistory() - 1).firstQuorumStake;
            uint96 eigenStakedBefore =
                dlReg.getTotalStakeFromIndex(dlReg.getLengthOfTotalStakeHistory() - 1).secondQuorumStake;
            ...
            uint96 ethStakedAfter =
                dlReg.getTotalStakeFromIndex(dlReg.getLengthOfTotalStakeHistory() - 1).firstQuorumStake;
            uint96 eigenStakedAfter =
                dlReg.getTotalStakeFromIndex(dlReg.getLengthOfTotalStakeHistory() - 1).secondQuorumStake;

            require(
                ethStakedAfter - ethStakedBefore == amountEthStaked, "eth quorum staked value not updated correctly"
            );
            require(
                eigenStakedAfter - eigenStakedBefore == amountEigenStaked,
                "eigen quorum staked value not updated correctly"
            );
        }
    }

    /// @notice Tests that registering the same public key twice reverts appropriately.
    function testRegisterPublicKeyTwice(uint8 operatorIndex) public fuzzedOperatorIndex(operatorIndex) {
    ...
    function testRegisterWhileAlreadyActive(uint8 operatorIndex, uint256 ethAmount, uint256 eigenAmount)
        public
        fuzzedOperatorIndex(operatorIndex)
    {
        cheats.assume(ethAmount > 0 && ethAmount < 1e18);
    ...
    function testOperatorDoesNotOwnPublicKey(uint8 operatorIndex, uint256 ethAmount, uint256 eigenAmount)
        public
        fuzzedOperatorIndex(operatorIndex)
    {
        cheats.assume(ethAmount > 0 && ethAmount < 1e18);
    ...
    function testRegisterForDataLayrWithNeitherQuorum(uint8 operatorIndex, uint256 ethAmount, uint256 eigenAmount)
        public
        fuzzedOperatorIndex(operatorIndex)
    {
        cheats.assume(ethAmount > 0 && ethAmount < 1e18);
        ...
        _testRegisterBLSPubKey(operatorIndex);
        cheats.expectRevert(
            bytes("RegistryBase._registrationStakeEvaluation: Must register as at least one type of validator")
        );
        _testRegisterOperatorWithDataLayr(operatorIndex, noQuorumOperatorType, testEphemeralKeyHash, testSocket);
    }
    /// @notice Tests that registering without adequate quorum stake reverts

    function testRegisterWithoutEnoughQuorumStake(uint8 operatorIndex) public fuzzedOperatorIndex(operatorIndex) {
        _testRegisterBLSPubKey(operatorIndex);
    ...
    function testRegisterWithZeroPubKey(uint8 operatorIndex, uint256 ethAmount, uint256 eigenAmount)
        public
        fuzzedOperatorIndex(operatorIndex)
    {
        cheats.assume(ethAmount > 0 && ethAmount < 1e18);
    ...
    function testRegisterWithoutSlashingOptIn(uint8 operatorIndex, uint256 ethAmount, uint256 eigenAmount)
        public
        fuzzedOperatorIndex(operatorIndex)
    {
        cheats.assume(ethAmount > 0 && ethAmount < 1e18);
        ...
        cheats.expectRevert(
            bytes("RegistryBase._addRegistrant: operator must be opted into slashing by the serviceManager")
        );
        _testRegisterOperatorWithDataLayr(operatorIndex, operatorType, testEphemeralKeyHash, testSocket);
    }
    ...
    function testRegisteringWithSamePubKeyAsAggPubKey(uint8 operatorIndex, uint256 ethAmount, uint256 eigenAmount)
        public
        fuzzedOperatorIndex(operatorIndex)
    {
        cheats.assume(ethAmount > 0 && ethAmount < 1e18);
        ...
        bytes memory packedAPK =
            abi.encodePacked(bytes32(prevAPK[1]), bytes32(prevAPK[0]), bytes32(prevAPK[3]), bytes32(prevAPK[2]));
