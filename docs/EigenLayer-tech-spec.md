...
These returns provide an economic incentive for stakers to opt-in and act as “operators” for services. In order to disincentivize malicious actions and deliver *cryptoeconomic security* to services, services built on EigenLayer also impose **slashing conditions** in which provably bad behavior is punished. 
...
New services built on EigenLayer can define their own, arbitrary *slashing conditions*. In particular, this allows services to potentially slash their operators for any action that is **on-chain checkable**. In particular, this is compatible with the permissionless ability to launch a new service on EigenLayer only because all services are opt-in; if a staker believes a service has an unsafe slashing mechanism, then they can simply not opt-in to serving that application.

## Actors in the System

### Stakers
A **staker** is any party who has assets deposited into EigenLayer. In general, these could be any mix of ERC20 tokens and/or ETH staked itself (deposited by transferring withdrawal credentials to EigenLayer). Stakers can delegate their stake to an operator, or act as an operator themselves.
...
### Services
We refer to software built on top of EigenLayer as **services**, or occasionally as **middleware**. Since we anticipate a wide variety of services built on top of EigenLayer, we have endeavored to make a minimal amount of assumptions about the struture of services.
...
We assume that such an honest watcher will fraudproof *all false claims*, regardless of the size and independent of any financial incentive that may or may not be present for the watcher. Efforts have been made to relax this assumption, but work is still ongoing.

### Discretization of Services ("Tasks")
We assume that services manage **tasks**. In other words, we assume that services discretize commitments undertaken by operators, which each define the time period for which their operators' stakes are placed "at risk", i.e. potentially subject to slashing.

### Delegation "Trust Network" Structure
It is assumed that any staker who delegates their stake to an operator is in the same "trust network" as their chosen operator. In other words, the staker-delegated-operator relationship is assumed to have a significant *trust component*. Operators may have the ability to steal the yield that they earn from the stakes of stakers who delegate to them, as well as imposing other negative externalities on those delegated to them.
...
Withdrawals and undelegation are handled through the `InvestmentManager`. both *necessitate delays*, since it is infeasible to immediately know whether or not specific restaked funds are "at stake" on any existing services. Instead, stakers who wish to withdraw and/or undelegate must go through a *queued withdrawal* process, in which they:
1. Signal that their funds should no longer be placed "at stake" on new tasks
2. Make a claim to the latest time at which their funds are indeed "at stake"
...
Assets may be depositable or withdrawable to a single `Investment Strategy` contract in multiple forms, and the strategy may actively or passively manage the funds.
...
The Delegation contract also stores a mapping with the total shares of each strategy delegated to each operator -- `operatorShares[operator][strategy]` stores the total number of shares in `strategy` delegated to `operator`
...
* The sum of `operatorShares[operator][strategy]` over all operators should *always* be lesser than or equal `investorStratShares[staker][strategy]` over all stakers (possibility of some stakers not being delegated)

Delegation defines when an operator is delegated or not, as well as defining what makes someone an operator
...
Similar to withdrawals, **undelegation** in EigenLayer necessitate a delay or clawback mechanism. To elaborate: if a staker is delegated to an operator, and that operator places the staker's assets 'at risk' on some task in which the operator *misbehaves* (i.e. acts in a slashable manner), it is critical that the staker's funds can still be slashed
* stakers can only undelegate by queuing withdrawal(s) for *all of their assets currently deposited in EigenLayer*, ensuring that all existing tasks on which a staker's currently deposited assets are actively at risk are resolved prior to allowing a different operator to place those same assets at risk on other tasks
