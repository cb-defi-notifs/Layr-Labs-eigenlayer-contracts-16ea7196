...
rule shouldUpdateDAOCommissionPercentage(env e, uint256 newCommision) {
    uint256 oldPercentage = daoCommissionPercentage();
    updateDAORevenueCommission(e, newCommision);
    uint256 newPercentage = daoCommissionPercentage();

    assert oldPercentage != newPercentage, "Update to same revenue percentage";
    assert newPercentage == newCommision, "Update unsuccessful";
}
/*
